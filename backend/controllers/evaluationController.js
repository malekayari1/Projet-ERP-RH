...
exports.getEvaluationsForChefEquipe = async (req, res) => {
  ...
  const evaluations = await Evaluation.find(filter)
    .populate("employeeId", "fullName email department supervisorId managerId")
    .populate("campaignId", "title startDate endDate customCriteria")
    .sort({ createdAt: -1 });
...
exports.submitEvaluationByChefEquipe = async (req, res) => {
  ...
  const evaluation = await Evaluation.findById(id).populate("campaignId");
  ...
  if (otherCriteria) {
    evaluation.otherCriteria = otherCriteria;
  }

  // Calculer le score
  const scoreResult = evaluation.calculateScore();

  // Gestion de la prime si score >= 80
  if (scoreResult.score >= 80) {
    if (!primeAmount) {
      return res.status(400).json({ message: "Le montant de la prime est requis pour un score supérieur à 80" });
    }
